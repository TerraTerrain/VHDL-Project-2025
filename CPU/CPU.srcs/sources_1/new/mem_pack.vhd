...
package body mem_pack is
    ...
    begin
        ...
        case mn is
            when LB => 
                return imm12 & rs1 & Func3LB & rd & OpLoad;
            when LBU =>
                return imm12 & rs1 & Func3LBU & rd & OpLoad;
            when LH   =>
                return imm12 & rs1 & Func3LH & rd & OpLoad;
            when LHU => 
                return imm12 & rs1 & Func3LHU & rd & OpLoad;
            when LW =>
                return imm12 & rs1 & Func3LW & rd & OpLoad;
            when SB =>
                return imm12(11 downto 5) & rs2 & rs1 & Func3SB & imm12(4 downto 0) & OpStore;
            when SH =>
                return imm12(11 downto 5) & rs2 & rs1 & Func3SH & imm12(4 downto 0) & OpStore;
            when SW =>
                return imm12(11 downto 5) & rs2 & rs1 & Func3SW & imm12(4 downto 0) & OpStore;
            when ADD => 
                return Func7ADD & rs2 & rs1 & Func3Arthm & rd & OpReg;
            when SUB => 
                return Func7SUB & rs2 & rs1 & Func3Arthm & rd & OpReg;
            when ADDI => 
                return imm12 & rs1 & Func3Arthm & rd & OpImm;
            when LUI => 
                return imm20 & rd & OpLUI;
            when AUIPC => 
                return imm20 & rd & OpAUIPC;
            when XORr => 
            ...
            when XORI => 
                return imm12 & rs1 & Func3XOR & rd & OpImm;
            when ORI => 
                return imm12 & rs1 & Func3OR & rd & OpImm;
            when ANDI => 
                return imm12 & rs1 & Func3AND & rd & OpImm;
            when SLLr => 
            ...
            when SLLI => 
                return Func7ShLog & shamt & rs1 & Func3SLL & rd & OpImm;
            when SRLI => 
                return Func7ShLog & shamt & rs1 & Func3SRL_SRA & rd & OpImm;
            when SRAI => 
                return Func7ShArthm & shamt & rs1 & Func3SRL_SRA & rd & OpImm;
            when SLT => 
                return Func7Set & rs2 & rs1 & Func3SLT & rd & OpReg;
            when SLTU => 
                return Func7Set & rs2 & rs1 & Func3SLTU & rd & OpReg;
            when SLTI => 
                return imm12 & rs1 & Func3SLT & rd & OpImm;
            when SLTIU => 
                return imm12 & rs1 & Func3SLTU & rd & OpImm;
            when BEQ =>
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BEQ & imm12(4 downto 1) & imm12(11) & OpBranch;
            when BNE => 
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BNE & imm12(4 downto 1) & imm12(11) & OpBranch;
            when BLT => 
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BLT & imm12(4 downto 1) & imm12(11) & OpBranch;
            when BGE => 
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BGE & imm12(4 downto 1) & imm12(11) & OpBranch;
            when BLTU => 
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BLTU & imm12(4 downto 1) & imm12(11) & OpBranch;
            when BGEU => 
                return imm12(12) & imm12(10 downto 5) & rs2 & rs1 & Func3BGEU & imm12(4 downto 1) & imm12(11) & OpBranch;
            when JAL => 
                return imm20 & rd & OpJump;
            when JALR => 
                return imm12 & rs1 & Func3JALR & rd & OpJumpReg;
        end case;    
    end toMemEntry;        
    ...
    begin
        ...
        word_loop: while success loop
            ...
            if v(1) = '@' then
                ...
                if rd_set = true then
                    if r1_set = true then
                        r2 := toRegAddrType(v(2 to v'right));
                    else r1 := toRegAddrType(v(2 to v'right));
                    end if;
                else
                    rd := toRegAddrType(v(2 to v'right));
                    rd_set := true;
                end if;
        ...
        if not(addr_ptr) then
            mem (to_integer(i)):= toMemEntry(mnemonic, r1, r2, rd, imm);
            exit line_loop when to_integer(unsigned(i)) = 2**MemoryAddrSize-1;
            i := i + 4;
            rd_set := false;
            r1_set := false;
        end if;
